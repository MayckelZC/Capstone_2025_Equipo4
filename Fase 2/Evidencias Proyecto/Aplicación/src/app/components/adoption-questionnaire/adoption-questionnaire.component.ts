...
export class AdoptionQuestionnaireComponent implements OnInit, OnDestroy {
  ...
  isStepValid(step: number): boolean {
    switch (step) {
      case 1:
        // Validar housingType y secureFencing
        const housingTypeValid = this.questionnaireForm.get('housingType')?.valid ?? false;
        const secureFencingValid = this.questionnaireForm.get('secureFencing')?.valid ?? false;

        // Solo validar landlordAllowsPets si la casa es alquilada
        const housingType = this.questionnaireForm.get('housingType')?.value;
        const landlordValid = housingType === 'rented'
          ? (this.questionnaireForm.get('landlordAllowsPets')?.valid ?? false)
          : true; // Si es casa propia, este campo no aplica

        return housingTypeValid && secureFencingValid && landlordValid;

      case 2:
        const petLivingSpaceValid = this.questionnaireForm.get('petLivingSpace')?.valid ?? false;
        const hoursAloneValid = this.questionnaireForm.get('hoursAlone')?.valid ?? false;
        const householdMembersValid = this.questionnaireForm.get('householdMembers')?.valid ?? false;
        const allergiesValid = this.questionnaireForm.get('allergies')?.valid ?? false;

        // Solo validar petLivingSpaceOther si seleccionó 'other'
        const petLivingSpace = this.questionnaireForm.get('petLivingSpace')?.value;
        const petLivingSpaceOtherValid = petLivingSpace === 'other'
          ? (this.questionnaireForm.get('petLivingSpaceOther')?.valid ?? false)
          : true;

        return petLivingSpaceValid && petLivingSpaceOtherValid && hoursAloneValid &&
          householdMembersValid && allergiesValid;

      case 3:
        const previousExpValid = this.questionnaireForm.get('previousExperience')?.valid ?? false;
        const otherPetsValid = this.questionnaireForm.get('otherPets')?.valid ?? false;
        const petFoodValid = this.questionnaireForm.get('petFood')?.valid ?? false;
        const vetAccessValid = this.questionnaireForm.get('veterinaryAccess')?.valid ?? false;
        const unexpectedExpensesValid = this.questionnaireForm.get('unexpectedExpenses')?.valid ?? false;

        // Solo validar detalles si respondió 'Sí' a experiencia previa u otras mascotas
        const previousExp = this.questionnaireForm.get('previousExperience')?.value;
        const otherPets = this.questionnaireForm.get('otherPets')?.value;

        const previousExpDetailsValid = previousExp === true
          ? (this.questionnaireForm.get('previousExperienceDetails')?.valid ?? false)
          : true;

        const otherPetsDetailsValid = otherPets === true
          ? (this.questionnaireForm.get('otherPetsDetails')?.valid ?? false)
          : true;

        return previousExpValid && previousExpDetailsValid && otherPetsValid &&
          otherPetsDetailsValid && petFoodValid && vetAccessValid && unexpectedExpensesValid;

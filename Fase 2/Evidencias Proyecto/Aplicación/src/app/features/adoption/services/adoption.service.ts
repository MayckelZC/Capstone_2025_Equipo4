...
export class AdoptionService {
  ...
  async createRequest(request: AdoptionRequest): Promise<void> {
    ...
    await this.firestore.collection('adoption-requests').doc(id).set({ ...request, id, creatorId: petData.creadorId });
    ...
    // Send confirmation notification to applicant (can view pet details)
  ...
  getLatestPendingRequests(limit: number = 5) {
    // Intentamos primero con la query optimizada (requiere índice)
    return this.firestore.collection<AdoptionRequest>('adoption-requests', ref =>
      ref.where('status', '==', 'pending')
  ...
  async approveRequest(request: AdoptionRequest): Promise<void> {
    ...
    try {
      ...
      await this.notificationService.create({
        userId: request.applicantId,
        title: 'Solicitud Aprobada',
        body: `¡Felicidades! Tu solicitud de adopción para ${petName} ha sido aprobada. ${creatorName} se pondrá en contacto contigo${creatorData?.telefono ? ' al ' + creatorData.telefono : ''} para coordinar la entrega. También puedes contactar directamente desde Mis Adopciones.`,
        type: 'adoption_request',
        read: false,

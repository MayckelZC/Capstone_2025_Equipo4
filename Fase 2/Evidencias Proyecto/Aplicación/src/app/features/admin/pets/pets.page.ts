...
export class PetsPage implements OnDestroy {
  ...
  searchTerm: string = '';
  currentFilter: string = 'all'; // all, available, adopted, visible, hidden
  ...
  fetchPets(infiniteScroll?: any) {
    ...
    this.petsSubscription = query.get().pipe(
      ...
      forkJoin(petsWithCreator).subscribe(newPets => {
        ...
        } else if (this.currentFilter === 'adopted') {
          filtered = filtered.filter(pet => pet.status === 'adopted');
        }
  ...
  async togglePetVisibility(pet: Adopcion) {
    ...
    try {
      ...
      this.logger.error('Error al cambiar la visibilidad de la mascota:', error);
  ...
  trackByPetId(index: number, pet: Adopcion): string {
    return pet.id!;
  }
}

...
export class HomePage implements AfterViewInit, OnDestroy {
  ...
  async reportAdopcion(adopcion: Adopcion) {
    ...
    const modal = await this.modalController.create({
      component: (await import('../report-modal/report-modal.page')).ReportModalPage,
      componentProps: {
  ...
  irAEntrega(adopcionId: string) {
    this.router.navigate(['/entrega-mascota', adopcionId]);
  }
  ...
  async findPetsNearMe() {
    try {
      ...
      const city = 'Santiago';
      this.router.navigate(['/advanced-search'], { queryParams: { location: city, lat: coordinates.coords.latitude, lng: coordinates.coords.longitude } });
    } catch (error) {

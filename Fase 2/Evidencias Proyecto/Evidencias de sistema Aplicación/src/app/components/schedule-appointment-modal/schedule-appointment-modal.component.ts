...
export class ScheduleAppointmentModalComponent implements OnInit {
  ...
  // Slots de 20 minutos - mañana y tarde
  allTimeSlots: string[] = [
    '08:00', '08:20', '08:40',
    '09:00', '09:20', '09:40',
    '10:00', '10:20', '10:40',
  ...
  readonly slotDurationMinutes = 20;
  ...
  loadAvailableSlots() {
    ...
    this.appointmentService.getBookedTimeSlotsForDate(startOfDay).subscribe(
      bookedSlots => {
        ...
        // Si el tipo de cita requiere más de 20 minutos, validar slots consecutivos
        const duration = this.selectedAppointmentType?.duration || 20;
        const slotsNeeded = Math.ceil(duration / this.slotDurationMinutes); // Cada slot es 20 min
        ...
        this.availableTimeSlots = validTimeSlots.filter(slot => {
          ...
          // Si solo necesita 1 slot (20 min o menos), ya está disponible
          if (slotsNeeded === 1) {
            return true;
          }
